...
def _surface(
    *,
    ...
    if treatment_skill not in route.selected_skills or treatment_skill in CONTROL_SKILLS:
        raise ValueError("treatment must be one additional registered and routed Method Skill")
    arms = (("control", CONTROL_SKILLS), ("treatment", (*CONTROL_SKILLS, treatment_skill)))
    ...
    cost = estimate_paired_skill_ablation_cost(
        pricing=pricing,
        profile=profile,
        replicate_count=3,
        arm_count=2,
    ...
    if per_run_cap is None or 6 * per_run_cap > max_total_cost_microusd:
    ...
    return _Surface(repository, profile, instruction, arms, registration)
