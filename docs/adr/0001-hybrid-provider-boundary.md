...
The harness must integrate mature backtest, broker, and market-data systems without
reimplementing their execution engines. Candidate systems expose different integration
...
Use one typed internal provider contract and permit native Python, MCP, HTTP, or gRPC
adapters behind it. Register every provider through a versioned capability manifest.
Separate declared from verified capability and require stream plus reconciliation support
...
- Existing engines can be adopted without making MCP a universal broker protocol.
